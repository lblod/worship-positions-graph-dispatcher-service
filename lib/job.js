...
import { INITIAL_DISPATCH_ENDPOINT } from "../config";
...
export async function loadJob(subject) {
  ...
  const job = parseResult(await query(queryJob, {},{ sparqlEndpoint: INITIAL_DISPATCH_ENDPOINT }))[0];
  if (!job) return null;
  ...
  const tasks = parseResult(await query(queryTasks, {},{ sparqlEndpoint: INITIAL_DISPATCH_ENDPOINT })).map(row => row.task);
  job.tasks = tasks;
...
export async function getJobs(jobOperationUri, statusFilterIn = [], statusFilterNotIn = []) {
  ...
  `;
  const result = await query(queryIsActive, {},{ sparqlEndpoint: INITIAL_DISPATCH_ENDPOINT });
