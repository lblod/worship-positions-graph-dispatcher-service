  ...
  async run() {
    if (await this.checkReadyToRun()) {
      if (this.queue.length > 0 && !this.executing) {
        ...
        try {
          this.executing = true;
          console.log(`${this.name}: Processing job for subject <${job.subject}>. Remaining in queue: ${remaining}.`);
          const startedAt = Date.now();
          await job.task();
          console.log(`${this.name}: Done with subject <${job.subject}> in ${Date.now() - startedAt}ms. Remaining in queue: ${this.queue.length}.`);
        }
        catch (error) {
